...
void print_char(Xuint32 BaseAddress, unsigned char string_s){
			...
			{
				VGA_PERIPH_MEM_mWriteMemory(BaseAddress + TEXT_MEM_OFF + cursor_position, (string_s-0x60));
			}
...
void set_font_size(Xuint32 BaseAddress, int color){

	VGA_PERIPH_MEM_mWriteMemory(BaseAddress + 0x0C, color);
}

void clear_text(Xuint32 BaseAddress, int position){


				VGA_PERIPH_MEM_mWriteMemory(BaseAddress + TEXT_MEM_OFF + cursor_position + position*4, ' ');
...
void print_string(Xuint32 BaseAddress, unsigned char string_s[], int lenght){
	int i;
		...
		for (i = 0; i < lenght; i++){
			...
			{
				VGA_PERIPH_MEM_mWriteMemory(BaseAddress + TEXT_MEM_OFF + cursor_position + i*4, (string_s[i]-0x60));
			}
		}
